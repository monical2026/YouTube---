...
- `panel.html?history=1` 承载历史页，同一扩展来源校验继续适用，无新增站点权限和网络端口。`openHistory` 从视频页传 videoId，从其他页面打开时恢复上次选择。
- `listHistory` 在后台通过 IndexedDB 游标逐条校验记录，只返回列表元数据和可检索的个人笔记字段，不返回整片逐字稿或 AI 回答。异常条目统计数量且保留原数据；读取失败明确报错。
- `useVideo(historyVideoId)` 直接按记录标识读取本地缓存，忽略播放上下文与原标签关闭通知，仍订阅 `recordChanged`。同一组件继续通过原有串行保存队列和 revision 校验写入。卸载后迟到响应不得回填。
- `VideoRecord.updatedAt` 为可选兼容字段，真实保存时记录当前时间。旧数据无需重写或清库，缺失时间可回退笔记更新时间；都没有则显示“日期未记录”。历史选择时间及上次选择保存在扩展本地存储，与笔记修订分离。
- 读取目录、选视频、切换语言及页签不触发字幕、翻译、分析或模型请求。AI 提问沿用现有明确点击发送流程。
- 切换记录先等待编辑器保存最新草稿；失败留在原处。提问、逐字稿编辑与导出弹窗需先关闭。取消仍保存草稿，完成才结束草稿。
- `openVideoTime` 校验视频标识与非负时间，复用同视频标签；原标签不存在时用明确的 YouTube 地址与时间创建标签。不覆盖其他视频页。
- 本版不增加知识库接口占位实现；已有 videoId、note.id、时间与选定内容导出作为后续出口基础，后续范围见 history-records-design.md。
